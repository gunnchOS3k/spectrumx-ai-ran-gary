...
| Decision | Rationale |
|----------|-----------|
| **Controller labeled rule-based baseline (RIC-style abstraction)** | Shipped Streamlit extension uses **detector-conditioned** heuristic actions and KPI deltas (`select_closed_loop_action` / `apply_action_to_kpis`). Gate 2 `twin_policies` is rule-based / SLSQP. Trained RL or contextual-bandit policies are **not** claimed as current. |
| **Gate 2 lives in `src/airan_research` not Streamlit** | Twin-conditioned policies run via CLI / `make test`; they do not replace the judged `evaluate()` contract. |
