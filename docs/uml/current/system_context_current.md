...
| | |
|---|---|
| **Status** | **Current** — post-project truth for repo boundaries |
| **Purpose** | Show actors, GitHub repo contents, local/cloud Streamlit, and **external runtime targets** (manifest-only in app). |
| **Source** | [`docs/uml/system_context_current.mmd`](../system_context_current.mmd) |

SpectrumX judges and collaborators interact with the repo and Streamlit; heavy simulators (Sionna RT, AODT, pyAerial/cuPHY, OTA) are **not** executed inside the hosted app — they appear as **export/manifest ingestion** paths aligned with `docs/EXTERNAL_RUNTIME_GAPS.md`.
