"""
Train v7 model: optimized feature extraction, full IQ data.

Key improvements over v6:
- Vectorized spectral kurtosis via batched FFT (no per-segment Welch loop)
- Process up to 10M IQ samples (vs 5M) using the saved time budget
- Batched percentile computation
- Same 30 features, retrained weights for consistency with new extraction
"""
...
def extract_features(iq: np.ndarray, sample_rate: float) -> np.ndarray:
    ...
    if n_segs >= 8:
        ...
        iq_windowed = iq_trimmed * window[np.newaxis, :]
        fft_out = np.fft.fft(iq_windowed, axis=1)
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("results/v7_model.json"))
    args = parser.parse_args()

    print(f"Loading labeled data (up to {MAX_IQ // 1_000_000}M IQ samples per file)...")
    X_iq, y = load_labeled(args.dataset_root)
    print(f"Labeled: {len(X_iq)} | Classes: {dict(zip(*np.unique(y, return_counts=True)))}")

    print(f"Extracting {len(FEATURE_NAMES)} features (vectorized)...")
    ...
    # Estimated time per file on server
    est_total = elapsed / len(X_iq) * 240
    print(f"\nEstimated server time: {est_total:.0f}s for 240 files")
